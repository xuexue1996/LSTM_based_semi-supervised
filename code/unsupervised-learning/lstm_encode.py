...
import encode_twitter

datasets = {'twitter': (encode_twitter.load_data, encode_twitter.prepare_data)}
dataset_name = "cr_100"
...
layers = {'lstm_encoder': (param_init_lstm, lstm_layer), 'lstm_decoder':(param_init_lstm, lstm_layer)}
...
def build_decoder(tparams, options, encode):
    ...
    proj = tensor.concatenate([tensor.shape_padleft(encode[0]), proj[:proj.shape[0]-1]])
...
def pred_error(f_pred, prepare_data, data, iterator, verbose=False):
    ...
    return valid_err
def encode_input(f_encode, prepare_data, data, verbose=False):
    x_source, x_target, mask_source, mask_target, y = prepare_data(data[0],data[1])
    encode=f_encode(x_source, mask_source)
    return encode
...
def train_lstm(
    ...
    saveto='lstm_new.npz',  # The best model will be saved there
    ...
    minlen=0,
    maxlen=3000,  # Sequence longer then this get ignored
    ...
    reload_model='amazon_100_short.npz',  # Path to a saved model we want to start from.
):
    ...
    f_encode = theano.function([x_source, mask_source], encode, name='f_encode')
        ...
        for eidx in range(max_epochs):
            ...
            for _, train_index in kf:
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    num = len(train[1])/100
                    train_encode = encode_input(f_encode, prepare_data, [train[0][0:num],train[1][0:num]])
                    encode_result = numpy.asarray(train_encode[1])
                    for i in range(1,99):
                        #train_err = pred_error(f_decode, prepare_data, train, kf)
                        train_encode = encode_input(f_encode, prepare_data, [train[0][num*i:num*(i+1)],train[1][num*i:num*(i+1)]])
                        print("train encode result")
                        print(len(train_encode[1][0]))
                        
                        encode_result = numpy.concatenate((encode_result,numpy.asarray(train_encode[1])),0)
                    train_encode = encode_input(f_encode, prepare_data, [train[0][num*(i+1):],train[1][num*(i+1):]])
                    encode_result = numpy.concatenate((encode_result,numpy.asarray(train_encode[1])),0)
                    with open('%s.npy'%dataset_name,'w') as f:
                        numpy.save(f,encode_result)
