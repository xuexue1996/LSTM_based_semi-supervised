...
#import imdb
dataset_name = "encoderesult/newencodeluanmatrain1"
...
def train_lstm(
    ...
    minlen=-1,
    maxlen=100000,  # Sequence longer then this get ignored
    batch_size=1,  # The batch size during training.512 64
    ...
    if reload_model:
        load_params('savemodel/changeencodeluanma4__0.npz', params)
  ...
  #  f_decode = theano.function([x_source, x_target, mask_source, mask_target], pred.argmax(axis=2), name='f_decode')
   ...
   #s fout  = open("log.txt","w")
    ...
    num = len(train[1])/50000
    print("!!!!!!!!!!!!!!!!!!!!!!!")
    print(num)
    train_encode = encode_input(f_encode, prepare_data, [train[0][0:num],train[1][0:num]])
    if(times == 0):encode_result = numpy.asarray(train_encode[1])
    else :encode_result = numpy.concatenate((encode_result,numpy.asarray(train_encode[1])),0)
